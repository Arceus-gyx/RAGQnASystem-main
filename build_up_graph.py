...
def create_all_relationship(client, all_relationship):
    def create_relationship(client, type1, name1, relation, type2, name2):
        order = (
            """match (a:%s{名称:"%s"}),(b:%s{名称:"%s"}) create (a)-[r:%s]->(b)"""
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="通过medical.json文件,创建一个知识图谱"
    )
    parser.add_argument(
        "--website", type=str, default="http://localhost:7474", help="neo4j的连接网站"
    )
    parser.add_argument("--user", type=str, default="neo4j", help="neo4j的用户名")
    parser.add_argument(
        "--password", type=str, default="wei8kang7.long", help="neo4j的密码"
    )
    ...
    for i, data in enumerate(all_data):
        ...
        if check:
            relationship.extend(
                [("疾病", disease_name, "疾病所需检查", "检查项目", ch) for ch in check]
            )
        ...
        if cure_department:
            relationship.append(
                ("疾病", disease_name, "疾病所属科目", "科目", cure_department[-1])
            )
        ...
        if symptom:
            relationship.extend(
                [("疾病", disease_name, "疾病的症状", "疾病症状", sy) for sy in symptom]
            )
        ...
        if cure_way:
            ...
            relationship.extend(
                [
                    ("疾病", disease_name, "治疗的方法", "治疗方法", cure_w)
    ...
    all_entity = {
        k: (list(set(v)) if k != "疾病" else v) for k, v in all_entity.items()
    }
