...
from vector_rag.qdrant_store import format_vector_context
...
def qwen_chat_completion(prompt, model=QWEN_DEFAULT_MODEL):
    if model not in ("qwen-plus", "qwen-turbo"):
...
def add_vector_context_to_prompt(prompt, vector_context):
    if not vector_context:
        return prompt
    rag_prompt = f"<提示>以下是从医学文档向量库检索到的补充资料：\n{vector_context}\n</提示>"
    return rag_prompt + prompt
...
"""
    rec_result = qwen_chat_completion(prompt, model=choice)
...
def add_lianxi_prompt(entity, lianxi, target, client):
    ...
    try:
        sql_q = "match (a:疾病{名称:'%s'})-[r:%s]->(b:%s) return b.名称" % (
            entity,
            lianxi,
            target,
        )
...
def generate_prompt(
    response, query, client, bert_model, bert_tokenizer, rule, tfidf_r, device, idx2tag
):
    entities = zwk.get_ner_result(
        bert_model, bert_tokenizer, query, rule, tfidf_r, device, idx2tag
    )
    # print(response)
    # print(entities)
    yitu = []
    ...
    if "药品" in response:
        if "疾病" in entities:
            prompt += add_lianxi_prompt(
                entities["疾病"], "疾病使用药品", "药品", client
            )
            yitu.append("查询疾病使用药品")
    if "宜吃食物" in response:
        if "疾病" in entities:
            prompt += add_lianxi_prompt(
                entities["疾病"], "疾病宜吃食物", "食物", client
            )
            yitu.append("查询疾病宜吃食物")
    if "忌吃食物" in response:
        if "疾病" in entities:
            prompt += add_lianxi_prompt(
                entities["疾病"], "疾病忌吃食物", "食物", client
            )
            yitu.append("查询疾病忌吃食物")
    if "检查项目" in response:
        if "疾病" in entities:
            prompt += add_lianxi_prompt(
                entities["疾病"], "疾病所需检查", "检查项目", client
            )
            yitu.append("查询疾病所需检查")
    if "查询疾病所属科目" in response:
        if "疾病" in entities:
            prompt += add_lianxi_prompt(
                entities["疾病"], "疾病所属科目", "科目", client
            )
    ...
    if "症状" in response:
        if "疾病" in entities:
            prompt += add_lianxi_prompt(
                entities["疾病"], "疾病的症状", "疾病症状", client
            )
            yitu.append("查询疾病的症状")
    if "治疗" in response:
        if "疾病" in entities:
            prompt += add_lianxi_prompt(
                entities["疾病"], "治疗的方法", "治疗方法", client
            )
            yitu.append("查询治疗的方法")
    if "并发" in response:
        if "疾病" in entities:
            prompt += add_lianxi_prompt(
                entities["疾病"], "疾病并发疾病", "疾病", client
            )
            yitu.append("查询疾病并发疾病")
    if "生产商" in response:
        try:
            sql_q = "match (a:药品商)-[r:生产]->(b:药品{名称:'%s'}) return a.名称" % (
                entities["药品"]
            )
            res = client.run(sql_q).data()[0].values()
            prompt += f"<提示>"
...
def main(is_admin, usname):
    ...
    with st.sidebar:
        ...
        if is_admin:
            ...
            uploaded_doc = st.sidebar.file_uploader(
                "导入医学文档到Qdrant", type=["pdf", "txt"]
            )
            if uploaded_doc is not None and st.sidebar.button("写入向量库"):
                upload_dir = os.path.join("tmp_data", "uploaded_docs")
                os.makedirs(upload_dir, exist_ok=True)
                upload_path = os.path.join(upload_dir, uploaded_doc.name)
                with open(upload_path, "wb") as f:
                    f.write(uploaded_doc.getbuffer())
                try:
                    result = ingest_file(upload_path, store=load_vector_store())
                    st.sidebar.success(
                        f"导入完成：{result['inserted']} 个 chunks 写入 Qdrant"
                    )
                except Exception as exc:
                    st.sidebar.error(f"Qdrant 导入失败：{exc}")
        if st.button("返回登录"):
    ...
    client = py2neo.Graph(
        "http://localhost:7474", user="neo4j", password="12345678", name="neo4j"
    )
        ...
        with st.chat_message(message["role"]):
            ...
            if message["role"] == "assistant":
                ...
                if show_prompt:
                    ...
                    with st.expander("点击显示向量检索结果"):
                        st.write(message.get("vector_prompt", ""))

    if query := st.chat_input(
        "Ask me anything!", key=f"chat_input_{active_window_index}"
    ):
        ...
        prompt, yitu, entities = generate_prompt(
            response,
            query,
            client,
            bert_model,
        ...
        response_placeholder.text("正在进行向量检索...")
        vector_debug_content = "未检索到 Qdrant 文档片段。"
        try:
            vector_store = load_vector_store()
            vector_results = vector_store.search(query, top_k=5)
            vector_context = format_vector_context(vector_results)
            if vector_context:
                vector_debug_content = vector_context
        except Exception as exc:
            vector_results = []
            vector_context = ""
            vector_debug_content = f"Qdrant 向量检索失败：{exc}"
            if show_prompt:
                st.warning(f"Qdrant 向量检索失败：{exc}")
        prompt = add_vector_context_to_prompt(prompt, vector_context)

        last = qwen_chat_completion(prompt, model=choice)
        response_placeholder.markdown(last)
        response_placeholder.markdown("")

        knowledge = re.findall(r"<提示>(.*?)</提示>", prompt)
        zhishiku_content = "\n".join(
            [f"提示{idx + 1}, {kn}" for idx, kn in enumerate(knowledge) if len(kn) >= 3]
        )
        ...
        current_messages.append(
            {
                ...
                "vector_prompt": vector_debug_content,
                "ent": str(entities),
            }
        )
